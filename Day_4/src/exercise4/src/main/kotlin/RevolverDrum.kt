...
class RevolverDrum<T : PistolBullet>(private val klass: KClass<T>) {
    val drum: MutableList<T?> = MutableList(6) { null }
    private var pointer: Int = 0

    companion object {
        inline operator fun <reified T : PistolBullet> invoke() = RevolverDrum(T::class)
    }
    ...
    fun ejectShotAndDamp() {
        for (i in 0 until drum.size) {
            if (drum[i]?.shot == true || drum[i]?.damp == true) {
                drum[i] = null
            }
        }
    }
